...
#
# async def post_llm_request(session, titles, dialog_id):
#     """Отправляет список названий вакансий в LLM для определения синонимов и основного названия."""
#     prompt = ("Вы эксперт в HR и нормализации названий должностей. Я предоставлю список названий вакансий, некоторые из которых могут быть синонимами или вариациями одной роли. "
#     "Ваша задача — сгруппировать эти названия в кластеры синонимов и выбрать одно *основное* название для каждой группы, которое чётко и профессионально описывает роль (по возможности — на английском, кроме строго русскоязычного контекста). "
#     "Верните результат в виде JSON-объекта, где ключами будут основные названия ролей, а значениями — списки всех синонимичных названий, включая оригинальные. "
#     "Пример ввода: ['Frontend Developer', 'Frontend-разработчик', 'Front-end Developer'] "
#     "Пример вывода: {\"Frontend Developer\": [\"Frontend Developer\", \"Frontend-разработчик\", \"Front-end Developer\"]} "
#     "Вот список названий вакансий:\n" + "\n".join(titles))
#     # prompt = (
#     #     "Вы эксперт в HR и нормализации названий должностей. Я предоставлю список названий вакансий, "
#     #     "некоторые из которых могут быть синонимами или вариациями одной роли. Ваша задача — сгруппировать "
#     #     "эти названия в кластеры синонимов и выбрать одно 'основное' название для каждого кластера, которое "
#     #     "лучше всего представляет роль. Основное название должно быть четким, профессиональным и, по возможности, "
#     #     "на английском языке (кроме случаев, когда контекст строго русскоязычный). "
#     #     "Верните результат в виде JSON-объекта, где каждое исходное название сопоставлено с основным. "
#     #     "Например, если входные данные: ['Frontend Developer', 'Frontend-разработчик', 'Front-end Developer'], "
#     #     "выход должен быть: "
...
#     """Обрабатывает все названия вакансий с помощью LLM."""
#     dialog_id = f"{USER_DOMAIN_NAME}_{str(uuid.uuid4())}"
#
#     async with aiohttp.ClientSession() as session:
#         # Отправляем запрос
...
#         return
#
#     # Обрабатываем все названия сразу
#     mappings = {}
#     try:
#         mappings = await process_titles(job_titles)
#         if not mappings:
#             logger.warning(f"Не удалось обработать названия, сохраняем в {FAILED_DATA_PATH}")
#             try:
#                 with open(FAILED_DATA_PATH, 'w', encoding='utf-8') as f:
#                     json.dump(job_titles, f, ensure_ascii=False, indent=4)
#                 logger.info(f"Необработанные данные сохранены в {FAILED_DATA_PATH}")
#             except Exception as e:
#                 logger.error(f"Ошибка записи в файл {FAILED_DATA_PATH}: {e}")
#             return
#     except Exception as e:
